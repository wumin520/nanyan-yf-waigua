  ...
  <div>
    ...
    <a-card title="保全记录">
      <div class="options_select_ clearfix">
        登记时间：<label class="active_">本月</label><label>近三个月</label
        ><label>近六个月</label>
        <a-range-picker class="mrg_l29" @change="onChange" />
        <div class="btn_wrap_">
      ...
      <a-divider />
      <div class="options_select_">
        保全类型：<label class="active_">全部</label><label>增加</label
        ><label>减少</label><label>编辑</label>
      </div>
      <a-divider />
      <div class="options_select_">
        办理状态：<label class="active_">办理结束</label><label>办理中</label>
      </div>
      <a-divider />
      <a-table :columns="columns1" :dataSource="data1"> </a-table>
    </a-card>
  </div>
</template>
